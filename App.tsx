...
import { generateStory, analyzeStoryInputs, generateImage, generateCohesiveImagePrompts, analyzeCharacterDescriptions } from './services/geminiService';
    ...
    const handleFilesUpload = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const fileProcessingPromises: Promise<void>[] = Array.from(files).map((file: File) => (async () => {
            ...
            if (fileName.endsWith('.zip')) {
                try {
                    ...
                    zip.forEach((relativePath, zipEntry: any) => {
                        ...
                        const entryPromise = (async () => {
                            ...
                            // FIX: Filter out macOS metadata files from ZIP archives.
                            const justFileName = entryName.split('/').pop() || '';
                            if (entryName.startsWith('__MACOSX/') || justFileName.startsWith('._')) {
                            ...
                            try {
                                if (['png', 'jpg', 'jpeg', 'webp', 'gif', 'bmp'].includes(extension)) {
                                    ...
                                    if (!base64) { // Ensure content is not empty
                                        ignoredFiles.push(entryName);
                                        return;
                                    }
                ...
                // FIX: Filter out macOS metadata files for individual uploads.
    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            ...
            if (textFiles.length > 0) {
                ...
                storyElements = await analyzeStoryInputs(textFiles, uploadedCharacterNames);
            } else {
                report += '--- SOURCE FILES ---\n';
            ...
            const finalPrompt = `You are a master creative writer and screenwriter. Your task is to write a new short movie script based ENTIRELY on the provided series context and a user-provided event.

**CRITICAL INSTRUCTION: ADHERE TO SOURCE MATERIAL**
The "Series Premise" below is your absolute source of truth. You MUST derive the genre, tone, characters, and world from this context. DO NOT invent new genres (like sci-fi) unless it is explicitly stated in the provided text.

**CRITICAL INSTRUCTION: CASTING AND PROTAGONISTS**
The following characters are the STAR PROTAGONISTS. They have specific visual references provided by the user:
[ ${uploadedCharacterNames.length > 0 ? uploadedCharacterNames.join(', ') : 'None'} ]

You MUST write the story centered around these specific characters.
1. They must have the most lines and action.
2. If the "Series Premise" below mentions other characters, demote them to supporting roles or replace them with these Mandatory Protagonists if appropriate.
3. The "Inspirational Event" should happen TO or BECAUSE OF these characters.

**Series Premise (Context):**
---
**Characters (Background info - prioritize Mandatory Protagonists over these):**
...
`;
            ...
            const analyzedCharacters = await analyzeCharacterDescriptions(characterDescriptionSource, uploadedCharacterNames);
        ...
        try {
            ...
            if (imageFolder) {
                for(let i = 0; i < generatedImages.length; i++) {
                    ...
                    // Find nearest dialogue logic
                    let dialogueText = "";
                    // Search forward first
                    for (let j = img.sceneIndex + 1; j < scriptData.scene_elements.length; j++) {
                        const el = scriptData.scene_elements[j];
                        ...
                        if (el.type === 'dialogue_block') {
                             ...
                             if (spoken) {
                                 dialogueText = `${dBlock.character.toUpperCase()}: ${spoken}`;
                                 break;
                             }
                        }
                    }
                    ...
                    if (!dialogueText) {
                        for (let j = img.sceneIndex - 1; j >= 0; j--) {
                            const el = scriptData.scene_elements[j];
                            if (el.type === 'scene_heading') break;
                            if (el.type === 'dialogue_block') {
                                 const dBlock = el as DialogueBlock;
    ...
    }, [scriptData, generatedImages, generationReport, generatedCharacterPortraits]);
