...
export async function analyzeStoryInputs(
    ...
    const prompt = `You are a strict and thorough story analyst. Your task is to perform a deep analysis of the following text file contents and extract the key elements for a movie script. You must read and synthesize information from the entire body of all provided texts.

    **CRITICAL INSTRUCTION: PRIMARY PROTAGONISTS**
    The user has explicitly provided visual assets for the following characters: ${mandatoryCharacters.length > 0 ? mandatoryCharacters.join(', ') : 'None'}.
    
    If these names appear in the text, you MUST identify them as the LEAD CHARACTERS.
    If the text contains generic roles (e.g., "A Hero", "The Captain"), ASSUME these mandatory characters fill those roles.
    Your analysis of the 'Characters' and 'Core Story' MUST center around these provided names. Any other characters found in the text should be considered secondary or supporting roles.

    Identify the following three components based on a comprehensive review of the provided text and the mandatory character constraints:
    1.  **Characters:** Who are the main characters? (Prioritize: ${mandatoryCharacters.join(', ')}). Describe them using information from the files.
    2.  **Core Story:** What is the overall premise or background of the series?
    3.  **Daily Theme:** What is a specific theme, setting, or event for today's episode?
...
export async function analyzeCharacterDescriptions(sourceText: string, mandatoryCharacters: string[] = []): Promise<AnalyzedCharacter[]> {
    ...
    **CRITICAL PRIORITY - MANDATORY CHARACTERS:**
    The following characters are the STARS of the show and MUST be included in your output list: ${mandatoryCharacters.length > 0 ? mandatoryCharacters.join(', ') : 'None'}.
    If their physical description is missing in the text, infer plausible details based on their name or role, but DO NOT OMIT THEM.
...
const scriptSchema = {
    type: Type.OBJECT,
    properties: {
// FIX: Updated model name in description
...
export async function generateStory(prompt: string): Promise<Script> {
    try {
        const response = await ai.models.generateContent({
// FIX: Updated deprecated model 'gemini-2.5-pro' to 'gemini-3-pro-preview' for a complex text task.
...
// FIX: Updated fallback source file to match the model change.
